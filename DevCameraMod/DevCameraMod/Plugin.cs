...
namespace DevCameraMod
{
    ...
    public class Plugin : BaseUnityPlugin
    {
        ...
        // AI camera follow
        public float findAnotherCooldown;
        public float changeTransformCooldown;
        public float ifNotTaggedCooldown;
        public float tripsWithGuy;
        public float scoreboardUpdate;
        public float lastTrackedSurvivor;
        public bool  spectatingSurvivor = true;

        // User interface
        public string   lobbyToEnter       = "LUCIO";
        ...
        public void FixedUpdate()
        {
            ...
            if (cameraMode == CameraModes.DefEnhanced && PhotonNetwork.InRoom)
                fixedClip = clipPlane * GorillaTagger.Instance.offlineVRRig.transform.localScale.y;
            ...
            if (cameraMode == CameraModes.SelectedPlayer)
                fixedClip = clipPlane * GorillaParent.instance.vrrigs[rigtofollow].transform.localScale.y;

            editClipPlane = cameraMode == CameraModes.Default
                                    ? Mathf.Lerp(editClipPlane, 0.01f,     0.075f)
                                    : Mathf.Lerp(editClipPlane, fixedClip, 0.075f);

            camera.nearClipPlane = editClipPlane;

            float fixedFOV = FOV;
            if (cameraMode == CameraModes.DefEnhanced && PhotonNetwork.InRoom)
                fixedFOV = FOV * GorillaTagger.Instance.offlineVRRig.transform.localScale.y;
            ...
            if (cameraMode == CameraModes.SelectedPlayer)
                fixedFOV = FOV * GorillaParent.instance.vrrigs[rigtofollow].transform.localScale.y;

            editFOV = cameraMode == CameraModes.Default
                              ? Mathf.Lerp(editFOV, 60,       0.075f)
                              : Mathf.Lerp(editFOV, fixedFOV, 0.075f);
            ...
            if (PhotonNetwork.InRoom)
            {
                ...
                if (Time.time >= scoreboardUpdate)
                {
                    ...
                    {
                        string col = GorillaParent.instance.vrrigs[i].setMatIndex == 0
                                             ? ColorUtility.ToHtmlStringRGBA(GorillaParent.instance.vrrigs[i]
                                                    .materialsToChangeTo[0].color)
                                             : "751C00";

                        bool isTalking = GorillaParent.instance.vrrigs[i].rigContainer.Voice.IsSpeaking;

                        if (i >= 5)
                            cameraUI.scoreboardText2.text +=
                                    $"<color=#{(isTalking ? "FFFFFF" : "8E8E8E")}>{i}.</color> <color=#{col}>{GorillaParent.instance.vrrigs[i].playerText1.text}</color>\n";
                        else
                            cameraUI.scoreboardText.text +=
                                    $"<color=#{(isTalking ? "FFFFFF" : "8E8E8E")}>{i}.</color> <color=#{col}>{GorillaParent.instance.vrrigs[i].playerText1.text}</color>\n";
                    }
            ...
            if (cameraUI.canvas.enabled)
            {
                ...
                if (timeStart) currentTime += Time.deltaTime;
                ...
                string fixedSeconds =
                        $"{(patchedMinutes != "0" ? patchedSeconds.Length == 1 ? string.Format("0{0}", patchedSeconds) : patchedSeconds : patchedSeconds)}";

                string fixedMilliseconds =
                        $"{(patchedMilliseconds.Length >= 2 ? patchedMilliseconds.Substring(0, 2) : patchedMilliseconds.Length == 1 ? string.Format("0{0}", patchedMilliseconds) : patchedMilliseconds)}";

                cameraUI.currentTime.text =
                        $"{(timeSpan.Minutes.ToString() == "0" ? "" : patchedHours == "0" ? string.Format("{0}:", timeSpan.Minutes) : string.Format("{1}:{0}:", patchedMinutes.Length == 1 ? string.Format("0{0}", patchedMinutes) : patchedMinutes, patchedHours))}{fixedSeconds}.{fixedMilliseconds}";
            }
            ...
            try
            {
                if (cameraMode == CameraModes.Freecam)
                {
                    ...
                    if (Keyboard.current.shiftKey.isPressed) currentMultiplier = 3f;
                    else currentMultiplier                                     = 1f;

                    if (Keyboard.current.wKey.isPressed)
                        rawVelocity += camera.transform.forward * currentSpeed * currentMultiplier * 0.015f;

                    if (Keyboard.current.sKey.isPressed)
                        rawVelocity += camera.transform.forward * currentSpeed * currentMultiplier * 0.015f * -1;

                    if (Keyboard.current.dKey.isPressed)
                        rawVelocity += camera.transform.right * currentSpeed * currentMultiplier * 0.015f;

                    if (Keyboard.current.aKey.isPressed)
                        rawVelocity += camera.transform.right * currentSpeed * currentMultiplier * 0.015f * -1;

                    if (Keyboard.current.qKey.isPressed)
                        rawVelocity += camera.transform.up * currentSpeed * currentMultiplier * 0.015f;

                    if (Keyboard.current.eKey.isPressed)
                        rawVelocity += camera.transform.up * currentSpeed * currentMultiplier * 0.015f * -1;

                    if (Keyboard.current.upArrowKey.isPressed)
                        rawEulerAngles +=
                                new Vector2(currentSpeed * currentMultiplier * 0.015f * -1 * rotationMultiplier, 0);

                    if (Keyboard.current.downArrowKey.isPressed)
                        rawEulerAngles += new Vector2(currentSpeed * currentMultiplier * 0.015f * rotationMultiplier,
                                0);

                    if (Keyboard.current.rightArrowKey.isPressed)
                        rawEulerAngles += new Vector2(0,
                                currentSpeed * currentMultiplier * 0.015f * rotationMultiplier);

                    if (Keyboard.current.leftArrowKey.isPressed)
                        rawEulerAngles += new Vector2(0,
                                currentSpeed * currentMultiplier * 0.015f * -1 * rotationMultiplier);

                    if (Gamepad.current != null)
                    {
                        Gamepad currentGamepad = Gamepad.current;
                        if (currentGamepad.leftStick.ReadValue().y > 0.1)
                            rawVelocity += camera.transform.forward * currentSpeed * currentMultiplier * 0.015f;

                        if (currentGamepad.leftStick.ReadValue().y < -0.1)
                            rawVelocity += camera.transform.forward * currentSpeed * currentMultiplier * 0.015f * -1;

                        if (currentGamepad.leftStick.ReadValue().x > 0.1)
                            rawVelocity += camera.transform.right * currentSpeed * currentMultiplier * 0.015f;

                        if (currentGamepad.leftStick.ReadValue().x < -0.1)
                            rawVelocity += camera.transform.right * currentSpeed * currentMultiplier * 0.015f * -1;

                        if (currentGamepad.rightStick.ReadValue().y > 0.1)
                            rawEulerAngles +=
                                    new Vector2(currentSpeed * currentMultiplier * 0.015f * -1 * rotationMultiplier, 0);

                        if (currentGamepad.rightStick.ReadValue().y < -0.1)
                            rawEulerAngles +=
                                    new Vector2(currentSpeed * currentMultiplier * 0.015f * rotationMultiplier, 0);

                        if (currentGamepad.rightStick.ReadValue().x > 0.1)
                            rawEulerAngles += new Vector2(0,
                                    currentSpeed * currentMultiplier * 0.015f * rotationMultiplier);
                        ...
                        if (currentGamepad.leftShoulder.isPressed)
                            rawVelocity += camera.transform.up * currentSpeed * currentMultiplier * 0.015f;

                        if (currentGamepad.rightShoulder.isPressed)
                            rawVelocity += camera.transform.up * currentSpeed * currentMultiplier * 0.015f;
                    }
                ...
                {
                    if (Keyboard.current.wKey.isPressed)
                        forward += currentSpeed * currentMultiplier * 0.015f / Time.deltaTime * 0.005f;

                    if (Keyboard.current.sKey.isPressed)
                        forward += currentSpeed * currentMultiplier * 0.015f / Time.deltaTime * -1 * 0.005f;

                    if (Keyboard.current.dKey.isPressed)
                        right += currentSpeed * currentMultiplier * 0.015f / Time.deltaTime * -1 * 0.005f;

                    if (Keyboard.current.aKey.isPressed)
                        right += currentSpeed * currentMultiplier * 0.015f / Time.deltaTime * 0.005f;

                    if (Keyboard.current.qKey.isPressed)
                        up += currentSpeed * currentMultiplier * 0.015f / Time.deltaTime * -1 * 0.005f;

                    if (Keyboard.current.eKey.isPressed)
                        up += currentSpeed * currentMultiplier * 0.015f / Time.deltaTime * 0.005f;
            ...
            if (cameraMode == CameraModes.ActivitySpan || cameraMode == CameraModes.LavaFocus ||
                    ...
                    if (toRig != null)
                    {
                        if (!firstperson)
                            try
                            {
                                ...
                                cameraUI.currentlySpectating.text =
                                        $"{toRig.playerText1.text}{Environment.NewLine}{(toRig.setMatIndex == 0 ? "SURVIVOR" : "TAGGER")}";

                                cameraUI.currentSpecImage.texture =
                                        toRig.materialsToChangeTo[toRig.setMatIndex].mainTexture ?? null;

                                cameraUI.currentSpecImage.color =
                                        toRig.materialsToChangeTo[toRig.setMatIndex].GetColor("_Color") == null
                                                ? Color.white
                                                : toRig.materialsToChangeTo[toRig.setMatIndex].GetColor("_Color");
                                ...
                                if (updateHideCosmetics)
                                {
                                    ...
                                    foreach (GameObject cosmetic in toRig.cosmetics)
                                        try
                                        {
                                            if (cosmetic.transform.parent.parent.name == toRig.headMesh.name)
                                                cosmetic.layer = 0;
                                        }
                                        catch (Exception) { }
                                }
                            ...
                            {
                                Debug.LogError(
                                        $"ERROR: Failed to spectate player in third person. {e.Message} {e.Source} {e.StackTrace}");
                            }
                        else
                            try
                            {
                                ...
                                if (lastRig != null && lastRig != toRig)
                                {
                                    ...
                                    foreach (GameObject cosmetic in lastRig.cosmetics)
                                        try
                                        {
                                            if (cosmetic.transform.parent.parent.name == toRig.headMesh.name)
                                                cosmetic.layer = 0;
                            ...
                            {
                                Debug.LogError(
                                        $"ERROR: Failed to spectate player in first person. {e.Message} {e.Source} {e.StackTrace}");
                            }
                    }
                }
                else
            ...
            if (cameraMode == CameraModes.SelectedPlayer)
                if (GorillaParent.instance.vrrigs[rigtofollow] != null)
                {
                    if (!firstperson)
                    {
                        ...
                        shouldBeEnabled         = true;
                        cameraUI.currentlySpectating.text =
                                $"{rig.playerText1.text}{Environment.NewLine}{(rig.setMatIndex == 0 ? "SURVIVOR" : "TAGGER")}";

                        cameraUI.currentSpecImage.texture =
                                rig.materialsToChangeTo[rig.setMatIndex].mainTexture ?? null;

                        cameraUI.currentSpecImage.color =
                                rig.materialsToChangeTo[rig.setMatIndex].GetColor("_Color") == null
                                        ? Color.white
                                        : rig.materialsToChangeTo[rig.setMatIndex].GetColor("_Color");
        ...
        protected void OnGUI()
        {
            if (ShowMenu)
            {
                ...
                if (ShowFrame)
                {
                    ...
                    bool lab = !PhotonNetwork.InRoom;

                    if (lab) GUI.Label(new Rect(35, 125, 180, 20), "<color=red>Join a room for all modes</color>");
                    ...
                    if (cameraMode != CameraModes.Default     && cameraMode != CameraModes.FP &&
                        cameraMode != CameraModes.DefEnhanced && cameraUI   != null)
                    {
                        ...
                        if (cameraMode == CameraModes.Freecam)
                        {
                            GUI.Label(new Rect(60, optionPosition, 160, 20),
                                    $"Speed: {(currentSpeed.ToString().Length >= 3 ? currentSpeed.ToString()[..3] : currentSpeed.ToString())}");

                            currentSpeed = GUI.HorizontalSlider(new Rect(25 + 10 / 2, optionPosition + 30, 170, 20),
                                    currentSpeed, 0, 5);
                        ...
                        cameraLerp = GUI.HorizontalSlider(new Rect(25 + 5, optionPosition + 30, 160 / 2, 20),
                                cameraLerp, 0.02f, 0.3f);

                        quatLerp = GUI.HorizontalSlider(new Rect(180 - 65, optionPosition + 30, 160 / 2, 20), quatLerp,
                                0.02f, 0.3f);
                        ...
                        clipPlane = GUI.HorizontalSlider(new Rect(180 - 65, optionPosition + 30, 160 / 2, 20),
                                clipPlane, 0.01f, 0.5f);

                        optionPosition += 40;
                        GUI.Label(new Rect(60, optionPosition, 160, 20),
                                $"Camera listener: {(listener ? "On" : "Off")}");

                        float listenerFloat = GUI.HorizontalSlider(new Rect(25 + 10 / 2, optionPosition + 30, 170, 20),
                                listener ? 1 : 0, 0, 1);

                        optionPosition += 40;
                        GUI.Label(new Rect(60, optionPosition, 160, 20), $"CGT sponsors: {(show ? "On" : "Off")}");
                        float sponsor = GUI.HorizontalSlider(new Rect(25 + 10 / 2, optionPosition + 30, 170, 20),
                                show ? 1 : 0, 0, 1);

                        optionPosition += 45;
                        GUI.Label(new Rect(45, optionPosition, 160, 60),
                                $"Left team: {cameraUI.LeftTeamName}\nRight team: {cameraUI.RightTeamName}");

                        optionPosition += 10;
                        string teamName = GUI.TextArea(new Rect(25 + 10 / 2, optionPosition + 30, 170, 20),
                                cameraUI.LeftTeamName, 200);

                        optionPosition += 22.5f;
                        string rightTeamName = GUI.TextArea(new Rect(25 + 10 / 2, optionPosition + 30, 170, 20),
                                cameraUI.RightTeamName, 200);

                        if (teamName.Length <= 7 && lastLeftName != teamName)
                        {
                            if (type != null) GorillaTagger.Instance.offlineVRRig.tagSound.PlayOneShot(type);
                            cameraUI.LeftTeamName  = teamName.Replace(Environment.NewLine, "");
                        ...
                        {
                            if (type != null) GorillaTagger.Instance.offlineVRRig.tagSound.PlayOneShot(type);
                            cameraUI.RightTeamName  = rightTeamName.Replace(Environment.NewLine, "");
                    ...
                    else
                    {
                        ...
                        if (!PhotonNetwork.InRoom)
                        {
                            ...
                            string lobbyTemp = GUI.TextArea(new Rect(25 + 10 / 2.5f, optionPosition + 30, 170, 20),
                                    lobbyToEnter, 200);
                            ...
                            string joinText;
                            string[] joiningNames = new string[3]
                                    { "JoiningPublicRoom", "JoiningSpecificRoom", "JoiningFriend", };

                            string[] loadingNames = new string[2]
                                    { "Initialization", "DeterminingPingsAndPlayerCount", };

                            if (PhotonNetworkController.Instance == null) joinText = "Connecting";
                            else
                                joinText = joiningNames.Contains(PhotonNetworkController.Instance.CurrentState())
                                                   ? "Cancel"
                                                   : loadingNames.Contains(
                                                           PhotonNetworkController.Instance.CurrentState())
                                                           ? "Connecting"
                                                           : "Join";
        ...
        public void OnInitialized()
        {
            ...
            camera        = GorillaTagger.Instance.thirdPersonCamera.transform.GetChild(0).GetComponent<Camera>();
            cameraParent  = GorillaTagger.Instance.thirdPersonCamera.transform;
            virtualCamera = camera.GetComponentInChildren<CinemachineBrain>();
            vrrig         = GorillaTagger.Instance.offlineVRRig;

            CanChangeMode = true;

            fixedCameraNames.Add("VR");
            fixedCameras.Add(GTPlayer.Instance.GetComponentInChildren<Camera>().cullingMask);
            fixedCameraNames.Add("Spectator");
            fixedCameras.Add(camera.cullingMask);

            cameraUI = new CameraUI();

            Stream str = Assembly.GetExecutingAssembly()
                                 .GetManifestResourceStream("DevCameraMod.Resources.devcameraui");
            ...
            playerListener               = GTPlayer.Instance.GetComponentInChildren<AudioListener>();
        ...
        private void UpdateThead()
        {
            ...
            try
            {
                ...
                {
                    Thread.Sleep(5000);
                    string webData = webClient.DownloadString(
                            "https://raw.githubusercontent.com/developer9998/DevCameraMod/main/DevCameraMod/DevCameraMod/ScreenText.txt");
        ...
        internal void SwitchModePress(bool leftButton, int minConstraints, int maxConstaints)
        {
            ...
            {
                if (PhotonNetwork.InRoom)
                    intMode = Mathf.Clamp(intMode - 1, 0,
                            PhotonNetwork.CurrentRoom.IsVisible ? minConstraints : maxConstaints);
        ...
        public void OnModeChange()
        {
            ...
            {
                follower.transform.SetParent(GTPlayer.Instance.headCollider.transform, false);
                follower.playerHead = GTPlayer.Instance.headCollider.transform;
            }

            if (cameraMode == CameraModes.SelectedPlayer || cameraMode == CameraModes.DefEnhanced   ||
                cameraMode == CameraModes.LavaFocus      || cameraMode == CameraModes.SurvivorFocus ||
                cameraMode == CameraModes.ActivitySpan) virtualCamera.enabled = true;
            ...
            if (GorillaGameManager.instance                                   != null &&
                GorillaGameManager.instance.GetComponent<GorillaTagManager>() != null)
...
#region Player Searching
        ...
        public bool CheckIfBiggerGroup(bool checkForSameGroup, bool checkForBiggerGroup)
        {
            ...
            List<VRRig> shuffledPlayers = GorillaParent.instance.vrrigs.OrderBy(a => rng.Next()).ToList();
            ...
            if (lastplr - monkesInArea <= Mathf.RoundToInt(PhotonNetwork.CurrentRoom.PlayerCount * 0.16f))
                return checkForSameGroup; // if the group is still intact
        ...
        public void FindRoundBasedGroup(bool infected)
        {
            ...
            if (gtm.GameModeName().ToLower().Contains("casual")) return;            // Requires the Infection gamemode
            if (gtm.currentInfected.Count == PhotonNetwork.CurrentRoom.PlayerCount) // Requires survivor gorillas
            {
                monkRand = new Random(); // Randomizes the random value after the round is finished
            ...
            try
            {
                ...
                infectedGorillas.AddRange(from infectedNetPlayer in gtm.currentInfected
                                          where infectedNetPlayer.GetRigByNetPlayer() != null
                                          select infectedNetPlayer.GetRigByNetPlayer());

                survivorGorillas.AddRange(
                        GorillaParent.instance.vrrigs.Where(gorilla => !infectedGorillas.Contains(gorilla)));
            }
            catch (Exception e)
            {
            ...
            if (infected)
            {
                ...
                foreach (VRRig t in infectedGorillas)
                {
                    VRRig lavaGorilla = t;
                    List<float> dist = survivorGorillas
                                      .Select(survivorGorilla =>
                                                      Vector3.Distance(survivorGorilla.headMesh.transform.position,
                                                              lavaGorilla.headMesh.transform.position)).ToList();

                    float totalDist = dist.Sum();
                ...
                if (lavaDistances.Count > 0)
                {
                    for (int i = 0; i < infectedGorillas.Count; i++)
                    {
                        ...
                        if (!Mathf.Approximately(dist, lavaDistances.Min()))
                            continue;

                        foreach (VRRig survivorGorilla in survivorGorillas.Where(survivorGorilla =>
                                         Vector3.Distance(survivorGorilla.headMesh.transform.position,
                                                 lavaGorilla.headMesh.transform.position) < 7.5f))
                        {
                            currentToFollow = lavaGorilla;
                            currentToStare  = survivorGorilla;
                            closestDist = Vector3.Distance(survivorGorilla.headMesh.transform.position,
                                    lavaGorilla.headMesh.transform.position);
                        }
                    ...
                    {
                        if (currentToFollow !=
                            currentToStare) // if the lava is focussing in on another player, try doing another few things to ensure this.
                        {
                            foreach (VRRig survivorGorilla in survivorGorillas)
                            {
                                if (currentToStare != survivorGorilla) // check to see if this move should be corrected
                                    continue;

                                List<float> distances = infectedGorillas
                                                       .Select(LavaGorilla =>
                                                                       Vector3.Distance(
                                                                               survivorGorilla.headMesh.transform
                                                                                      .position,
                                                                               LavaGorilla.headMesh.transform.position))
                                                       .ToList();
                        ...
                        else // if they're alone, try to look again for others.
                        {
                            ...
                            try
                            {
                                foreach (VRRig SurvivorGorilla in survivorGorillas)
                                {
                                    ...
                                    {
                                        VRRig LavaGorilla = infectedGorillas[i];
                                        localDistances.Add(Vector3.Distance(SurvivorGorilla.headMesh.transform.position,
                                                LavaGorilla.headMesh.transform.position));
                ...
                if (currentToFollow != null)
                {
                    if (toRig != null)
                    {
                        float distBetween = Vector3.Distance(currentToStare.headMesh.transform.position,
                                toRig.headMesh.transform.position);
                    ...
                    if (toRig == currentToFollow)
                    {
                        if (currentToStare != currentToFollow)
                            if (Vector3.Distance(currentToStare.headMesh.transform.position,
                                        currentToFollow.headMesh.transform.position) > 5)
            ...
            else
            {
                ...
                for (int i = 0; i < survivorGorillas.Count; i++)
                {
                    ...
                    for (int index = 0; index < infectedGorillas.Count; index++)
                    {
                        ...
                        dist.Add(Vector3.Distance(SurvivorGorilla.headMesh.transform.position,
                                LavaGorilla.headMesh.transform.position));
                    }
                ...
                if (lavaDistances.Count > 0)
                {
                    for (int i = 0; i < survivorGorillas.Count; i++)
                    {
                        ...
                        if (dist == lavaDistances.Min())
                        {
                            ...
                            {
                                VRRig SurvivorGorilla = infectedGorillas[index];
                                if (Vector3.Distance(SurvivorGorilla.headMesh.transform.position,
                                            LavaGorilla.headMesh.transform.position) < 7.5f)
                                {
                                    currentToFollow = LavaGorilla;
                                    currentToStare  = SurvivorGorilla;
                            ...
                            currentToStare) // if the lava is focussing in on another player, try doing another few things to ensure this.
                        {
                            for (int index = 0; index < infectedGorillas.Count; index++)
                            {
                                ...
                                if (currentToStare == SurvivorGorilla) // check to see if this move should be corrected
                                {
                                    ...
                                    {
                                        VRRig LavaGorilla = survivorGorillas[i];
                                        distances.Add(Vector3.Distance(SurvivorGorilla.headMesh.transform.position,
                                                LavaGorilla.headMesh.transform.position));
                                    }
        ...
        public void FindGroup()
        {
            ...
            foreach (VRRig rig in shuffledPlayers)
            {
                ...
                for (int i = 0; i < recordedPositions.Count; i++)
                    if (positionFound != recordedPositions[i])
                        if (Vector3.Distance(positionFound, recordedPositions[i]) <
                            Mathf.Clamp(PhotonNetwork.CurrentRoom.PlayerCount * 0.5f, 1, 5))
                        {
                            monkesInArea++;
                            if (monkesInArea >=
                                Mathf.Clamp(Mathf.RoundToInt(PhotonNetwork.CurrentRoom.PlayerCount * 0.225f), 1, 3))
                            {
                                if (lookforMore)
                                {
                                    toRig          = rig;
            ...
            if (lookforMore && toRig == null)
                toRig = shuffledPlayers[
                        shuffledPlayers.Count - 1]; // if theres no groups and theres no rig, go to a random rig
        }
